...
public class FS-21_Findinterleavingofgivenstrings {
...
class InterleavingStringsFormattedInput{
    ...
    public static void back(String a,String b ,String temp ,int i,int j,List<String> res){
        if(a.length()==i && b.length()==j){
            res.add(temp);
            return ;
        }
        if(i<a.length()){
            back(a,b,temp+a.charAt(i),i+1,j,res);
        }
        if(j<b.length()){
            back(a,b,temp+b.charAt(j),i,j+1,res);
        }
    }
}