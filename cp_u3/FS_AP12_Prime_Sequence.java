...
class Solution {
    ...
    // Check if substring is a prime sequence
    public static boolean isPrimeSequence(String sub, String s) {
        int divisor = Integer.parseInt(sub);  // Convert substring to integer
        int dividend = Integer.parseInt(s);  // Convert full number to integer

        return (dividend % divisor == 0) && isPrime(divisor);
    }

    // Count prime-sequence beauty
    public static int getBeauty(String s, int k) {
        int count = 0;
        
        for (int i = 0; i <= s.length() - k; i++) {
            String sub = s.substring(i, i + k); // Extract substring of length k
            if (isPrimeSequence(sub, s)) {
                count++;
            }
        }
        return count;
    ...
    public static void main(String[] args) {
        ...
        System.out.println(getBeauty(s, k));
    }
}
